...
future_dojo_event_infos = []
future_dojo_events = get_future_dojo_events()

for future_dojo_event in future_dojo_events:
    future_dojo_event_infos.append(get_dojo_info(future_dojo_event))

if 1 == len(future_dojo_event_infos):

    future_dojo_event_info = future_dojo_event_infos[0]
    ...
    if not os.path.exists(dojo_filename):
        with open(dojo_filename, "w") as dojo_file:
            dojo_file.write(template
                            ...
                            .replace("{{event_description}}", replacenth(future_dojo_event_info['event_description'], "</p>\n<p>", "</p>\n\n<!--more-->\n\n<p>", 2))
                            )
            print("created " + dojo_filename)
    else:
        print(f"page {dojo_filename} already exists")
        quit(10)
