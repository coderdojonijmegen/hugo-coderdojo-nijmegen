...
for event in events:
    print(f"{datetime.strftime(event['event_date'], '%m')} - {'online' if event['online_event'] else event['venue']['name']} - {event['name']['text']} - {event['capacity']}, {get_sold_tickets(event)}, {get_checkins(event)}")
