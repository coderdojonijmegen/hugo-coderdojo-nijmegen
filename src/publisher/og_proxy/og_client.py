...
def fetch_og_data(url: str) -> dict:
    ...
    icon_urls = as_dict([(tag.get("rel")[0], full_url(domain, tag.get("href"))) for tag in link_tags if
                         tag.get("rel") and "icon" in tag.get("rel")])
    return {
        "url": url,
        "remote_response_code": r.status_code,
        "domain": domain,
        "title": raw_og_data["og:title"] if "og:title" in raw_og_data else raw_og_data["twitter:title"],
        "description": raw_og_data["og:description"] if "og:description" in raw_og_data else raw_og_data[
            "twitter:description"],
